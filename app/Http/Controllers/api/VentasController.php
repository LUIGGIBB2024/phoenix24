...
class VentasController extends Controller
{
    public function UpdateSales(Request $request):JsonResponse
    {
        ...
        $contador = 0;
        if (isset($request->detmiscelaneos))
        {
           $detmiscelaneos  = $request->detmiscelaneos;
           foreach($detmiscelaneos as $dato)
           {
             $codigoid        =  is_null($dato['codigoid'])?"":$dato['codigoid'];
             $codigo          =  is_null($dato['codigo'])?"":$dato['codigo'];
             if ($codigoid != "")
             {
            
                $reg_info  = detalledemiscelaneo::updateOrCreate(['codigoid'=>$codigoid,'codigo'=>$codigo],
                [
                'descripcion'                   => is_null($dato['descripcion'])?"":$dato['descripcion'],  
                'porcentaje1'                   => is_null($dato['porcentaje1'])?0:$dato['porcentaje1'],
                'porcentaje2'                   => is_null($dato['porcentaje2'])?0:$dato['porcentaje2'],
                'porcentaje3'                   => is_null($dato['porcentaje3'])?0:$dato['porcentaje3'],
                'porcentaje4'                   => is_null($dato['porcentaje4'])?0:$dato['porcentaje4'],
                'porcentaje5'                   => is_null($dato['porcentaje5'])?0:$dato['porcentaje5'],
                'valorfijo'                     => is_null($dato['valorfijo'])?0:$dato['valorfijo'],
                'factor1'                       => is_null($dato['factor1'])?0:$dato['factor1'],
                'factor2'                       => is_null($dato['factor2'])?0:$dato['factor2'],
                'factor3'                       => is_null($dato['factor3'])?0:$dato['factor3'],
                'factor4'                       => is_null($dato['factor4'])?0:$dato['factor4'],
                'factor5'                       => is_null($dato['factor5'])?0:$dato['factor5'],
                'codigodepartamento'            => is_null($dato['codigodepartamento'])?"":$dato['codigodepartamento'],
                'codigomunicipio'               => is_null($dato['codigomunicipio'])?"":$dato['codigomunicipio'],
                'nit001'                        => is_null($dato['nit001'])?"":$dato['nit001'],
                'nit002'                        => is_null($dato['nit002'])?"":$dato['nit002'],
                'nit003'                        => is_null($dato['nit003'])?"":$dato['nit003'],
                'nit004'                        => is_null($dato['nit004'])?"":$dato['nit004'],
                'nit005'                        => is_null($dato['nit005'])?"":$dato['nit005'],
                'campouti001'                   => is_null($dato['campouti001'])?"":$dato['campouti001'],
                'campouti002'                   => is_null($dato['campouti002'])?"":$dato['campouti002'],
                ...
                'cuenta03'                      => is_null($dato['cuenta03'])?"":$dato['cuenta03'],
                'cuenta04'                      => is_null($dato['cuenta04'])?"":$dato['cuenta04'],
                'cuenta05'                      => is_null($dato['cuenta05'])?"":$dato['cuenta05'],
                'centro'                        => is_null($dato['centro'])?"":$dato['centro'],
                'scentro'                       => is_null($dato['scentro'])?"":$dato['scentro'],
                'tipodemovimiento'              => is_null($dato['tipodemovimiento'])?0:$dato['tipodemovimiento'],
                'tipodeaplicacion'              => is_null($dato['tipodeaplicacion'])?0:$dato['tipodeaplicacion'],
                'tipodeformato'                 => is_null($dato['tipodeformato'])?0:$dato['tipodeformato'],
                'tipomvto01'                    => is_null($dato['tipomvto01'])?0:$dato['tipomvto01'],
                'tipomvto02'                    => is_null($dato['tipomvto02'])?0:$dato['tipomvto02'],
                'tipomvto03'                    => is_null($dato['tipomvto03'])?0:$dato['tipomvto03'],
                'tipomvto04'                    => is_null($dato['tipomvto04'])?0:$dato['tipomvto04'],
                'tipomvto05'                    => is_null($dato['tipomvto05'])?0:$dato['tipomvto05'],
                'estado01'                      => is_null($dato['estado01'])?0:$dato['estado01'],
                'estado02'                      => is_null($dato['estado02'])?0:$dato['estado02'],
                'estado03'                      => is_null($dato['estado03'])?0:$dato['estado03'],
                'estado04'                      => is_null($dato['estado04'])?0:$dato['estado04'],
                'estado05'                      => is_null($dato['estado05'])?0:$dato['estado05'],
                'identif01'                     => is_null($dato['identif01'])?"":$dato['identif01'],
                'identif02'                     => is_null($dato['identif02'])?"":$dato['identif02'],
                'identif03'                     => is_null($dato['identif03'])?"":$dato['identif03'],
                'identif04'                     => is_null($dato['identif04'])?"":$dato['identif04'],
                'identif05'                     => is_null($dato['identif05'])?"":$dato['identif05'],
                'documentoanexo'                => is_null($dato['documentoanexo'])?"":$dato['documentoanexo'],
                'MiscelaneosID'                 => 1,
