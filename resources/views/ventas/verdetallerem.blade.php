...
<div class="app-page-title">
    <h4 id = "encab_pagina">Detalle de Remisión N°::{{ $remision->consecutivo}} :: Valor $:{{number_format($remision->totaldocumento)}}{{"/".$remision->nombreventa }}</h4>
                            ...
                            @foreach ($detalles as $item )
                                <tr role="row" class="">
                                    @php
                                       ...
                                       $iva    =  ($subtotal - $dscto1 - $dscto2) * (0 +($item->ivaproducto/100));
                                       $total  =  round(($subtotal - $dscto1 - $dscto2 + $iva),0);
                                    @endphp
                                    <td > {{ $item->DetalleDeRemisionID}} </td>
                                    <td> {{ $item->producto}} </td>
                                    ...
                                    <td style= "text-align: right;"> {{ number_format($item->ivaproducto)}}</td>
